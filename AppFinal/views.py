    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:    #si ha encontrado un usuario con eso datos
                ...
                return render(request, "AppFinal/inicio.html", {'mensaje':f"Hola {user} has iniciado sesion correctamente."}) 
        else:   #si el formulario no es valido (no encuentra usuario)
        #mostramos la página de inicio junto a un mensaje de error.
            return render(request, "AppFinal/inicio.html", {'mensaje':"Error. Datos ingresados son incorrectos. Por favor intente nuevamente."})
    else:
...
def registrarse(request):
    if request.method == 'POST':    #cuando le haga click al botón
        ...
        if form.is_valid(): 
            ...
            form.save() #guardamos los datos ingresados en el formulario.
...
@login_required  #para la edicion de usuarios debe iniciar sesion.
...
#vista para editar las reseñas, requiere iniciar sesion. 
@login_required
...
#vista para eliminar reseñas, requiere iniciar sesion.
@login_required
